...
#ifndef FIFO_math_accel_p_channel_HH_
#define FIFO_math_accel_p_channel_HH_

#include <systemc>
using namespace std;
SC_MODULE(FIFO_math_accel_p_channel) {
    static const unsigned int DATA_WIDTH = 32;
    static const unsigned int ADDR_WIDTH = 2;
    static const unsigned int FIFO_math_accel_p_channel_depth = 3;
    ...
    sc_core::sc_signal< sc_dt::sc_lv<DATA_WIDTH> >  mStorage[FIFO_math_accel_p_channel_depth];
    ...
    SC_CTOR(FIFO_math_accel_p_channel) : mTrace(0) {
        ...
        SC_METHOD(proc_dout);
        sensitive << mOutPtr;
        for (unsigned i = 0; i < FIFO_math_accel_p_channel_depth; i++) {
        ...
        if (reset.read() == sc_dt::SC_LOGIC_1) {
            ...
            if (if_read_ce.read() == sc_dt::SC_LOGIC_1
               ...
               && internal_empty_n.read() == sc_dt::SC_LOGIC_1) {
                ...
                if (mOutPtr.read().to_uint() == (FIFO_math_accel_p_channel_depth-1)) {
                    ptr = 0;
                ...
                assert(ptr.to_uint() < FIFO_math_accel_p_channel_depth);
                mOutPtr.write(ptr);
            }
            if (if_write_ce.read() == sc_dt::SC_LOGIC_1
                ...
                if (ptr.to_uint() == (FIFO_math_accel_p_channel_depth-1)) {
                    ptr = 0;
                    mFlag_nEF_hint.write(~mFlag_nEF_hint.read());
                } else {
                    ptr++;
    ...
    void proc_dout() {
        ...
        if (ptr.to_uint() > FIFO_math_accel_p_channel_depth) {
...
#endif //FIFO_math_accel_p_channel_HH_
