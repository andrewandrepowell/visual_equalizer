/*
 * Purpose: Receive and send audio samples at 48 kHz,
 * buffer samples, and execute DMA when buffer is full.
...
#define FFT_CONFIG_VAL_SCALE_SCH		( 0b0000000000 ) /* bits are in pairs. */
...
int main()
{
	...
	xil_printf( "Starting application...\n\r" );
	{
		...
		{
			/* The memory needs to be accessible by CPU1. */
			Xil_SetTlbAttributes( CPU_1_START_REG, STRONG_ORDERED|SHAREABLE );

			/* Now, let's set the start address. */
			volatile uint32_t& start_reg = *reinterpret_cast<volatile uint32_t*>( CPU_1_START_REG );
			start_reg = CPU_1_START_ADDR;

			/* Signal an event so that CPU1 wakes up immediately. */
			__asm__ __volatile__ ( "sev" );

			/* Block until software interrupt from CPU1. */
		...
		while ( true )
		{
			...
			if ( getbufferset().getinput().push( ( samples[ 0 ] >> 12 ) & 0xFFFF ) )
			{
				int Status;
