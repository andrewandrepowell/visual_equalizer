	...
	audio_i2c_trans perform_i2c_trans, audio_i2c_delay perform_delay, void* param )
{
	...
	ptr->perform_delay = perform_delay;
	ptr->param = param;
...
void audio_i2c_configure( audio* ptr, audio_src src )
{
	audio_i2c_delay perform_delay = ptr->perform_delay;
	void* param = ptr->param;
	uint16_t mask = 0b000010000;
	...
	audio_i2c_write_reg( ptr, AUDIO_REG_ANA_AUD_PATH,	mask ); 	   /* Connect DAC to Output. Connect Mic to ADC. */
